...
def test_train_component(mocker, dataset_train_test_table, test_dataset, model_artifact):
    ...
    mocker.patch(
        "xgb-churn-model.data.data_split.split_X_y",
        return_value=("X", "y"),
    )
    mocker.patch(
        "xgb-churn-model.data.data_ingestion.execute_bq_query",
        return_value=test_dataset,
    )

    mocker.patch("xgb-churn-model.model.train.train_model", return_value="MOCK")
